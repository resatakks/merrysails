...
import {
  PHONE,
  PHONE_DISPLAY,
  getContactChannelWithMessage,
} from "@/lib/constants";

export default function WhatsAppButton() {
  const pathname = usePathname() ?? "/";
  // Strip optional locale prefix (e.g. /tr/, /de/) so localized tour pages also
  // get the lifted offset that keeps the button above the booking bar.
  // RU is included so Russian visitors also get the lift on tour-detail pages.
  const localeMatch = pathname.match(/^\/(tr|de|fr|nl|ru)(?=\/|$)/);
  const locale = localeMatch?.[1] ?? "en";
  const normalizedPath = pathname.replace(/^\/(tr|de|fr|nl|ru)(?=\/|$)/, "") || "/";
  ...
  // Locale-aware messaging channel. WhatsApp has been blocked in Russia since
  // Feb 2026 — every wa.me deep link silently fails for /ru visitors. Route
  // them to Telegram instead. All other locales keep WhatsApp (still the
  // dominant Bosphorus-tour intent channel).
  const channel = getContactChannelWithMessage(
    locale,
    "Hello, I'm interested in your Bosphorus cruise tours.",
  );
  const isTelegram = channel.icon === "telegram";
  const channelBgClass = isTelegram
    ? "bg-[#229ED9] hover:bg-[#1B85B8]"
    : "bg-[#25D366] hover:bg-[#20BD5A]";
  const pingBgClass = isTelegram ? "bg-[#229ED9]" : "bg-[#25D366]";
  const ariaLabel = isTelegram ? "Chat on Telegram" : "Chat on WhatsApp";
      ...
      >
        ...
        <a
          ...
          onClick={(event) =>
            handleTrackedContactNavigation(event, {
              href: channel.url,
              // Analytics kind union is closed to "phone" | "whatsapp"; the
              // channel is encoded in `label` so /ru clicks still surface in
              // dashboards as `sticky_telegram_button` instead of being lost.
              kind: "whatsapp",
              label: `sticky_${channel.icon}_button`,
              location: pathname,
            })
          }
        ...
        >
          ...
          {isTelegram ? (
            // Lucide's Send (paper plane) is the closest visual stand-in for
            // Telegram in the icon set; the brand-blue background carries the
            // recognition. Swap to an inline SVG path if a tighter brand match
            // is required later.
            <Send className="relative z-10 h-6 w-6 text-white md:h-7 md:w-7" aria-hidden />
          ) : (
