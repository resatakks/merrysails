  ...
  return (
    <Link href={`/cruises/${tour.slug}`} className="tour-card block group">
      <div className="relative aspect-[4/5] overflow-hidden">
        ...
        <span className={`absolute top-4 left-4 px-3 py-1.5 rounded-lg text-xs font-bold z-10 ${tour.badgeColor}`}>{tour.badge}</span>
        <div className="absolute top-4 right-4 z-10 price-badge"><span className="currency">€</span><span className="amount">{tour.priceEur}</span></div>
        <div className="card-content absolute bottom-0 left-0 right-0 p-5 z-10">
          <h3 className="font-heading text-lg font-bold text-white leading-snug">{tour.name}</h3>
