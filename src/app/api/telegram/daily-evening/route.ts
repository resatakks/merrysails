...
import { formatEveningSummary } from "@/lib/telegram/formatters";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const start = new Date(Date.UTC(trNow.getUTCFullYear(), trNow.getUTCMonth(), trNow.getUTCDate() + 1, -3, 0, 0));
    const end = new Date(Date.UTC(trNow.getUTCFullYear(), trNow.getUTCMonth(), trNow.getUTCDate() + 1, 20, 59, 59));
