...
export const metadata: Metadata = {
 ...
 description:
 "Sunset cruise tickets Istanbul from €30 (Mon, Tue & Thu). Golden-hour Bosphorus cruise — package options, ticket clarity, and direct booking from Karakoy waterfront.",
 alternates: {
...
const faqItems = [
 ...
 a: "The current public structure shows two shared sunset options on the same route: Without Wine from EUR 30 (Mon, Tue & Thu) / EUR 34 (other days) and With Wine from EUR 35 (Mon, Tue & Thu) / EUR 40 (other days).",
 },
 {
...
const faqItemsRu = [
 ...
 a: "Билеты на закатный круиз: пакет без вина — от 30 EUR (понедельник, вторник, четверг) или 34 EUR (другие дни). Пакет с вином — от 35 EUR (пн/вт/чт) или 40 EUR. Все пакеты — у лицензированного оператора TÜRSAB Группы А. Уточните доступность в WhatsApp.",
 },
 {
 q: "От какого причала отправляется закатный круиз?",
 a: "Закатный круиз отправляется с пирса Каракёй (Karaköy) на европейской стороне Босфора. Добраться можно трамваем T1 до остановки Karaköy или такси (10 минут от Таксима). Точное место сбора подтверждается в WhatsApp после бронирования.",
 },
 {
 q: "Что включено в билет на закатный круиз?",
 a: "В билет включены: 2 часа на яхте, англоязычный гид, безалкогольные напитки и закуски, виды на дворец Долмабахче, Босфорский мост и Румелийскую крепость. Пакет с вином добавляет неограниченное вино. Лицензия TÜRSAB Группы А. Подробности в WhatsApp.",
 },
 {
 q: "Нужно ли бронировать билет заранее или можно купить на месте?",
 a: "Рекомендуем бронировать заранее — особенно на выходные и в высокий сезон (май–октябрь) места заполняются за 1–2 дня. Билеты на месте часто недоступны. Забронировать можно через сайт или напрямую в WhatsApp.",
 },
];
...
const ticketRows = [
 ["Without Wine", "from EUR 30 (Mon, Tue & Thu) / EUR 34", "Guests who want the shared sunset route with light hospitality and no wine service"],
...
export default function SunsetCruiseTicketsIstanbulPage() {
 ...
 <p className="text-3xl font-bold text-[var(--heading)]">from EUR 30 / EUR 35 (Mon, Tue & Thu)</p>
