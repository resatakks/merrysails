...
> **🎉 2026-07-17 FV-1 RESOLVING (yarın KAPAT):** /reservation dead-click Clarity 3g **%6.9 (29 sess)** — 07-15'teki %36.36 (11 sess) sağlam N ile net çöktü. Muhtemel sebep: yeni commit **fd88234** "stop Event Yacht 90 from routing into a doomed reservation flow" (07-16, canlı) + önceki CRO fix'ler (breadcrumb-mute 02a1e47 + package-card c98ca49/b169c81). Yarın holds ederse FV-1 KAPAT. **boutique-yacht-12 FV-10 sinyali TEMİZLENDİ** (taze veride sinyal yok, %22'lik 2/9 N=1 noise'du). **FV-9 /yacht holds** (6.25%/48 band-içi). **🆕 YENİ P1 blocker: GSC OAuth token EXPIRED** (merkezi + proje `invalid_grant`) → A7 inspection + B2 striking + B11 delta bloke, operatör refresh gerek (`scripts/gsc-oauth-refresh.mjs`); B1 28g merkezi cache çalışıyor. **🆕 İZLE: rez cancel-rate %37.5** (6/16 FRESH DB vs carry %16.7 = doubled) — 3 yacht iptali 07-14 CRO-block ÖNCESİ (price-block değil), baseline stale → yarın true prev-7d. **A1 FRESH:** CNI 1720, index-rate 15.6% (BASELINE, method değişti). Rich our-side CLEAN (r28 0-ihlal, itemReviewed live); Google-side verify token-blocked. R-block: R1 Bing RECOVERED-SUSTAINED (imp42), R2 GSC +4% temiz.
